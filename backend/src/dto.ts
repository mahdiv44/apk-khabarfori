import {IsString,MinLength,MaxLength,IsOptional,IsEmail,IsIn,IsBoolean,IsArray,ArrayMaxSize,IsUrl} from 'class-validator';
...
export class EmployeeDto{@IsString() @MinLength(2) @MaxLength(100) fullName!:string;@IsString() @MinLength(2) @MaxLength(100) position!:string;@IsIn(['مدیریت','تحریریه','خبرنگاری','روابط عمومی','فنی','پشتیبانی']) department!:string;@IsString() @MaxLength(2000) biography!:string;@IsEmail() email!:string;@IsOptional() @IsString() @MaxLength(30) phone?:string}
